    ...
    <article class="request-card werkbon-card ${n?"is-expanded":""}" data-werkbon-row-id="${c(e.id)}" tabindex="0">
      <div class="request-card-head">
        <div class="request-main">
          <span class="plate-chip">${c(ve(e.licensePlate))}</span>
...
`)}const x=`https://wa.me/${encodeURIComponent(a)}?text=${encodeURIComponent(ue)}`;if(window.open(x,"_blank","noopener,noreferrer"),r&&v)try{await Ge(t.invoice,{status:t.invoice.status!=="paid"?"link_sent":t.invoice.status,payment_link:v,payment_link_sent_at:new Date().toISOString(),payment_method:"mollie"});const V=new Date().toISOString(),F=M.findIndex(me=>me.id===t.invoice.id);F!==-1&&(M[F]={...M[F],status:M[F].status!=="paid"?"link_sent":M[F].status,paymentLink:v,paymentLinkSentAt:V,paymentMethod:"mollie"})}catch{}else it(t.invoice.id,"link_sent");return P("WhatsApp bericht geopend ✓","success"),!0},Kt=async(t,r)=>{var J,ue;const a=await Ke(r),o=!!a.customer_email,i=!!Je(a.customer_phone),s=String((I==null?void 0:I.quote_prefix)??((J=n.activeGarage)==null?void 0:J.quotePrefix)??"OFF-"),d=Number((I==null?void 0:I.quote_counter)??((ue=n.activeGarage)==null?void 0:ue.quoteCounter)??1),v=Number.isFinite(d)&&d>0?d:1,S=`${s}${String(v).padStart(3,"0")}`,L=String(a.factuurnummer||a.invoice.invoiceNumber||a.invoice.id||"factuur").trim(),T=async()=>{const x=await Qe();return Mt({jsPDFCtor:x,invoice:{...a.invoice,invoiceNumber:L},customerEmail:a.customer_email,customerPhone:a.customer_phone,garageProfile:I,pdfSettings:_e,documentType:"factuur",documentNumber:L})},U=async()=>{const x=await Qe();return Mt({jsPDFCtor:x,invoice:{...a.invoice,invoiceNumber:S},customerEmail:a.customer_email,customerPhone:a.customer_phone,garageProfile:I,pdfSettings:_e,documentType:"offerte",documentNumber:S})};return yt({triggerButton:t,title:"📄 PDF Acties",options:[{id:"download",icon:"⬇️",title:"Downloaden",description:"Sla de factuur op als PDF",onSelect:async x=>{x.setLoading("download",!0);try{const R=await T(),Q=`${L||"factuur"}.pdf`,V=URL.createObjectURL(R),F=document.createElement("a");F.href=V,F.download=Q,F.click(),URL.revokeObjectURL(V),x.forceClose(),P("PDF wordt gedownload ✓","success")}catch{P("PDF genereren mislukt","error")}finally{x.setLoading("download",!1)}}},{id:"quote-download",icon:"🧾",title:"Offerte downloaden",description:`Sla een offerte op als PDF (${S})`,onSelect:async(x,R)=>{x.setLoading(R,!0);try{const Q=await U(),V=`${S||a.invoice.id||"offerte"}.pdf`,F=URL.createObjectURL(Q),me=document.createElement("a");me.href=F,me.download=V,me.click(),URL.revokeObjectURL(F),x.forceClose(),P("Offerte PDF wordt gedownload ✓","success")}catch{P("Offerte PDF genereren mislukt","error")}finally{x.setLoading(R,!1)}}},{id:"print",icon:"🖨️",title:"Afdrukken",description:"Print direct naar printer",onSelect:async(x,R)=>{x.setLoading(R,!0);try{const Q=await T(),V=URL.createObjectURL(Q),F=window.open(V,"_blank","noopener,noreferrer");if(!F)throw new Error("Popup blocked");F.addEventListener("load",()=>{F.print(),F.addEventListener("afterprint",()=>{try{F.close()}catch{}URL.revokeObjectURL(V)},{once:!0})},{once:!0}),x.forceClose(),P("Print dialoog geopend ✓","success")}catch{P("Afdrukken mislukt","error")}finally{x.setLoading(R,!1)}}},{id:"email",icon:"📧",title:"Stuur via E-mail",description:o?"Verstuur naar klant e-mail":"⚠️ Geen e-mailadres bekend. Voeg een e-mailadres toe aan deze werkbon.",disabled:!o,onSelect:async(x,R)=>{x.setLoading(R,!0);const Q=await Ve(a);x.setLoading(R,!1),Q&&(x.forceClose(),q())}},{id:"whatsapp",icon:"💬",title:"Stuur via WhatsApp",description:i?"Deel alleen de factuur via WhatsApp (zonder betaallink)":"⚠️ Geen telefoonnummer bekend. Voeg een telefoonnummer toe.",disabled:!i,onSelect:async(x,R)=>{x.setLoading(R,!0);const Q=await qe(a,{includePaymentLink:!1});x.setLoading(R,!1),Q&&(x.forceClose(),q())}}]})},Vt=async(t,r)=>{var v;const a=await Ke(r),o=a.invoice,i=!!Je(a.customer_phone),d=String(((v=I||n.activeGarage)==null?void 0:v.mollieMethod)||"none")!=="none";return yt({triggerButton:t,title:"💳 Betaalmethode",subtitle:`Totaal te betalen: €${Xe(a.total)}`,options:[{id:"cash",icon:"💵",title:"Contant",description:"Registreer contante betaling",onSelect:async S=>{S.setBodyContent(`
            ...
            `,L=>{const T=L.querySelector("[data-popup-cancel]"),U=L.querySelector("[data-popup-confirm]");T==null||T.addEventListener("click",()=>S.forceClose()),U==null||U.addEventListener("click",async()=>{await lt(o.id,t),S.forceClose(),q(),P("Contante betaling geregistreerd ✓","success")})})}},{id:"mollie_whatsapp",icon:"💬",title:"Betaallink via WhatsApp",description:d?i?"Stuur iDEAL link naar klant":"⚠️ Geen telefoonnummer bekend. Voeg een telefoonnummer toe.":"⚠️ Geen Mollie account verbonden. Ga naar Instellingen → Betalingen.",disabled:!d||!i,onSelect:async(S,L)=>{S.setLoading(L,!0);const T=await qe(a);S.setLoading(L,!1),T&&(S.forceClose(),q(),P("Betaallink verstuurd via WhatsApp ✓","success"))}}]})},Ut=(t,r)=>{_();const a=t.getBoundingClientRect(),o=document.createElement("div");o.className="send-dropdown",o.setAttribute("data-send-dropdown","true"),o.innerHTML=`
