...
<path d="M8.6665 12C8.6665 12 12.6665 9.05407 12.6665 8C12.6665 6.94587 8.6665 4 8.6665 4" stroke="#666666" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  `;const o=t.querySelector("#metricTotal"),L=t.querySelector("#metricToday"),M=t.querySelector("#metricCompleted"),$=t.querySelector("#metricEmails"),h=t.querySelector("#dashboardScheduleList"),g=t.querySelector("#dashboardOverviewLegend"),N=t.querySelector("#dashboardOverviewChart");try{const l=await P({garageIds:n}),f=H(l),u=l.filter(i=>i.inAppointments===!0),x=l.filter(i=>C(i.status)==="done"),B=k(new Date),b=u.filter(i=>k(i.appointmentAt??i.createdAt)===B).sort((i,w)=>new Date(i.appointmentAt??i.createdAt).getTime()-new Date(w.appointmentAt??w.createdAt).getTime());o.textContent=String(u.length),L.textContent=String(b.length),M.textContent=String(x.length),$.textContent=String(f.unread),h.innerHTML=G(b);const m=Y(u);g.innerHTML=W(m),O(N,m.labels,m.values),s(f.unread)}catch(l){h.innerHTML='<article class="schedule-row"><p class="muted">Unable to load dashboard data.</p></article>',g.innerHTML='<li><span class="muted">Unable to load overview.</span></li>',s(0),console.error(l)}}const F=document.querySelector("#app");D();Z(F);
