...
Message: Manual appointment created in dashboard.`,appointmentAt:`${K}T${_}`}),d.reset();const m=W();D(U(m)),N(`${String(m.getHours()).padStart(2,"0")}:${String(m.getMinutes()).padStart(2,"0")}`),w instanceof HTMLInputElement&&(w.value=""),T&&window.clearTimeout(T),o==null||o.abort(),h(b()),v(""),S("Appointment added successfully.","success"),await Y()}catch(m){const F=m instanceof Error?m.message:"Unable to add appointment.";v(F,"error"),S(F,"error")}finally{f instanceof HTMLButtonElement&&L&&(f.disabled=!1)}}),await Y()}const be=document.querySelector("#app");ee();fe(be);
