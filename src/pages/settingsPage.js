...
{{garage_naam}}</textarea>
  ...
  `}async function ot(A){var Pe,Le,Ee;if(!A)return;const n=await Ve();if(!n)return;if(!n.isAdmin&&!n.activeGarage){A.innerHTML=`
    ...
    `;return}We(n.activeGarage);const{shell:V,contentArea:e}=Re({activePage:"settings",title:"Instellingen",headerNote:"",userEmail:n.user.email,onLogout:ze,garage:n.activeGarage,isAdmin:n.isAdmin});A.replaceChildren(V);let O=(()=>{try{return localStorage.getItem("settings-active-folder")||"overview"}catch{return"overview"}})(),i={},X={},w={};const ce={dashboard:!1,website:!1,garage:!1,factuur:!1,offerte:!1,betalingen:!1},me=(t,r)=>{ce[t]=!!r},ge=()=>Object.values(ce).some(Boolean),_e=()=>confirmDialog("Je hebt wijzigingen gemaakt zonder op Opslaan te klikken. Weet je zeker dat je deze pagina wilt verlaten?","Niet-opgeslagen wijzigingen"),showAlert=(t,r="Melding")=>new Promise(n=>{const p=document.querySelector(".confirm-dialog-overlay");p&&p.remove();const u=document.createElement("div");u.innerHTML=`
      <div class="confirm-dialog-overlay" role="presentation">
        <div class="confirm-dialog" role="alertdialog" aria-modal="true">
          <div class="confirm-dialog-content">
            <h2 class="confirm-dialog-title">${Ie(r)}</h2>
            <p>${Ie(t)}</p>
          </div>
